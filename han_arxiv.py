# han_arxiv.py
# ------------------------------------------------------------
# Pure HAN on ArXiv (PyTorch Geometric), robust to local schema:
# - Loads ArXiv data using dataloader.py
...
def main():
    # 0) Load ArXiv data using dataloader
    data, (idx_train, idx_val, idx_test), generate_node_features, metapaths = load_data(dataset='ArXiv', return_mp=True)
    ...
    # 2) Create train/val/test masks for paper nodes
    category = data.category  # Should be 'paper'
    num_papers = data[category].num_nodes
    ...
    #    PCP: paper -> cites -> paper (self-loop)
    ...
    if ('paper', 'written_by', 'author') in edge_types and ('author', 'writes', 'paper') in edge_types:
        metapaths_node_pairs.append([('paper', 'author'), ('author', 'paper')])
    # Note: For self-loop citations, we don't need a separate metapath as it's already a direct edge
    ...
    # 6) Build HAN (2 layers) for 'paper' classification
