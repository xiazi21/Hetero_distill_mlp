# han_dblp.py
# ------------------------------------------------------------
# Pure HAN on DBLP (PyTorch Geometric), robust to local schema:
# - Prints node/edge types to help debugging
# - Maps 'venue' -> 'conference' if needed
# - Verifies/creates reverse edges required by meta-paths
...
def main():
    # 0) Load raw DBLP (no transforms)
    path = osp.join(osp.dirname(osp.realpath(__file__)), 'data', 'DBLP')
    data: HeteroData = DBLP(path)[0]

    # 1) Inspect schema
    print("[DEBUG] Node types:", data.node_types)
    print("[DEBUG] Edge types:", data.edge_types)

    # 2) Resolve node type aliases used by AddMetaPaths
    #    DBLP often uses 'conference' instead of 'venue'.
    venue_like = 'venue' if 'venue' in data.node_types else ('conference' if 'conference' in data.node_types else None)
    if venue_like is None:
        raise RuntimeError("Neither 'venue' nor 'conference' node type exists in this DBLP build.")

    term_like = 'term' if 'term' in data.node_types else None
    if term_like is None and 'keyword' in data.node_types:
        term_like = 'keyword'
    if term_like is None:
        # Some builds may use 'term', 'keyword', or 'topic'. Try 'topic':
        if 'topic' in data.node_types:
            term_like = 'topic'
    if term_like is None:
        raise RuntimeError("No 'term'/'keyword'/'topic' node type found in this DBLP build.")

    # 3) Define metapaths using *node-type pairs* expected by AddMetaPaths
    #    APA:   author -> paper -> author
    #    APVPA: author -> paper -> venue_like -> paper -> author
    #    APTPA: author -> paper -> term_like  -> paper -> author
    metapaths = [
        [('author', 'paper'), ('paper', 'author')],
        [('author', 'paper'), ('paper', venue_like), (venue_like, 'paper'), ('paper', 'author')],
        [('author', 'paper'), ('paper', term_like),  (term_like,  'paper'), ('paper', 'author')],
    ]
    print("[DEBUG] Using metapaths (node-type pairs):", metapaths)
    ...
    print("[DEBUG] After AddMetaPaths - edge types:", data.edge_types)

    # 6) Build HAN (2 layers) for 'author' classification
    category = 'author'
